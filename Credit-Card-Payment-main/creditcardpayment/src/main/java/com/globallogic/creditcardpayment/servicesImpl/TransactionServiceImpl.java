...
public class TransactionServiceImpl implements TransactionService {
	...
	public String updateTransaction(Transaction transaction) {
		...
		if (transactionRepo.findById(transaction.getTransid()).get() == null) {
...
//					statementRepo.setAmountForStatementWhenTransactionUpdated(dueAmount, s.getBillingDate(),
//							transaction1.getCustomer().getId());
//				}
//			}

			// Generating pdf for transaction details
			String path = pdfGenerator.transactionForCustomer(transaction.getTransid(), updateBalance);
//			List<Transaction> tr = transactionRepo.findAllTransactionTranDate(s1.getBillingDate().plusDays(-31),
//					s1.getBillingDate(), customer.getId(), "SUCCESSFULL");
//			String path1 = pdfGenerator.showAllTransactionsAndStatementForCustomerIdAndBillingDatePdf(tr,
//					customer.getId(), s1.getBillingDate().plusDays(-31), s1.getStatementid(), dueAmount);
...
//						path1);
	...
	public String deleteTransaction(long id) {
		if (transactionRepo.findById(id).get() == null) {
			throw new GlobalException();
		}
