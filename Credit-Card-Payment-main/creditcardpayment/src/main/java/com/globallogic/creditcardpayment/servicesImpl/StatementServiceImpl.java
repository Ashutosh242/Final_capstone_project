...
public class StatementServiceImpl implements StatementService {
	...
	public String updateStatement(Statement statement) {
		if (statementRepo.findById(statement.getStatementid()).get() == null) {
	...
	public String deleteStatement(long id) {
		if (statementRepo.findById(id).get() == null) {
			throw new GlobalException();
		}
	...
	public String showUnbilledStatementForCustomer(long custId) {
		...
		if (currentDate.isAfter(startDate)) {
			...
			List<Transaction> tr = transactionRepo.findAllTransactionTranDate(startDate.plusDays(-30), currentDate,
					custId, "SUCCESSFULL");
			for (Transaction t : tr) {
				sum = sum + t.getAmount();
			}
			...
			try {
				emailSenderService.sendEmailWithAttachment(customer.getEmail(),
						"UnBilled Statement details attached with mail",
						"UNBILLED STATEMENT GENERATED FOR " + currentDate.getMonth() + " FOR " + customer.getName(),
						path);
			} catch (MessagingException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
