...
public class ExceptionHandlingControllerAdvice extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<String> handleEmptyInput(GlobalException emptyInputException) {
		return new ResponseEntity<String>("ANY INPUT FIELD DETAILS IS EMPTY,PLEASE CHECK", HttpStatus.BAD_REQUEST);
	}
	
	@ExceptionHandler(MissingInputException.class)
	public ResponseEntity<String> handleMissingInput(MissingInputException missingInputException) {
		return new ResponseEntity<String>("ANY INPUT FIELD IS MISSING,PLEASE CHECK", HttpStatus.BAD_REQUEST);
	}
